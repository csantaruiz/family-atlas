...
describe('mapCameraGestures', () => {
  it('keeps the focal map point under the pinch midpoint', () => {
    ...
    const before = mapPointFromScreenPercent(originLeft, originTop, start)
    const zoomed = zoomCameraAt(start, 1.35, originLeft, originTop)
    const after = mapPointFromScreenPercent(originLeft, originTop, zoomed)
  ...
  it('clamps scale to the gesture range', () => {
    const tooFar = zoomCameraAt(DEFAULT_CAMERA, 40, 50, 50)
    expect(tooFar.scale).toBeLessThanOrEqual(5.5)
    const tooClose = zoomCameraAt(DEFAULT_CAMERA, 0.01, 50, 50)
  ...
  it('keeps the camera from leaving family content', () => {
    ...
    const clamped = clampCameraToContent({ cx: -40, cy: 200, scale: 1.4 }, bounds)
